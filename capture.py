...
direktori='/media/fishqi/FISHQ_STORE/RECORDING/'
...
logger.debug(f"capturing in {args.FPS} FPS/*")
#set parameter
fps = args.FPS
...
class Main():
    ...
    def run(self):
        ...
        with dai.Device(pipeline) as device:
            ...
            Path(direktori+f'{folderName}_color_data').mkdir(parents=True, exist_ok=True)
            Path(direktori+f'{folderName}_right_data').mkdir(parents=True, exist_ok=True)
            Path(direktori+f'{folderName}_left_data').mkdir(parents=True, exist_ok=True)
            ...
            while True:
                ...
                if inRight is not None:
                    ...
                    cv2.imwrite(direktori+f"{folderName}_right_data/{int(time.time()*10000)}.png",inRight.getCvFrame())

                if inLeft is not None:
                    cv2.imshow("fishQ:MONO-KIRI",inLeft.getCvFrame())
                    cv2.imwrite(direktori+f"{folderName}_left_data/{int(time.time()*10000)}.png",inLeft.getCvFrame())

                for encFrame in qJpeg.tryGetAll():
                    with open(direktori+f"{folderName}_color_data/{int(time.time() * 10000)}.jpeg", "wb") as f:
